...
//	m_embeddedCompInfoIndex = NULL; //埋め込んだ部品の情報へのポインタ
	m_useEmbeddedLib = true;
	m_pinltrb	=0;					//LTRBにピンが存在しているかどうかのフラグ
...
SXBSchComponent& SXBSchComponent:: operator=(const SXBSchComponent& comp)
{
	if(this != &comp){
		...
		*((SXBSchObj*)this) = comp;
		...
		m_dir			= comp.m_dir;			//向き
		m_pinltrb		= comp.m_pinltrb;		//LTRBにピンが存在しているかどうかのフラグ
		m_note			= comp.m_note;
...
//		componentwidth = (m_compInfoIndex->size().w()*10); 
	...
	textcenter = m_name_pos.x() + m_name.length()*8/2 - componentcenter;	//現在位置
	textcenter = -textcenter;	//左右反転した位置
	textX = textcenter - m_name.length()*8/2 + componentcenter;
	m_name_pos.setX(textX);


	textcenter = m_refnum_pos.x() + m_refnum.length()*8/2 - componentcenter;	//現在位置
	textcenter = -textcenter;	//左右反転した位置
