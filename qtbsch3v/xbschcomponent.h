...
public:
    ...
    bool qRedraw(const SRect&) {
        return true;   //描画が必要かどうかを返す qRedraw()は常に描画を強制する。
    }
...
public:
    ...
    bool nameHorizontal() const {
        return m_name_horiz;   //2005/04/09
    }
    ...
    void setNamePos(const SPoint& p) {
        m_name_pos = p;
    }
    ...
    const char* refnum() const {
        return m_refnum.c_str();
    }
    void setRefnumHorizontal(bool bHoriz);//2005/04/09
    bool refnumHorizontal() const {
        return m_refnum_horiz;   //2005/04/09
    }
    void setRefnumPos(const SPoint& p) {
        m_refnum_pos = p;
    }
    ...
    const char* orgname() const {
        return m_orgname.c_str();
    }
    const SCompIndex* compInfoIndex() const {
        return m_compInfoIndex;
    }
    ...
    bool canAttribute() {
        return true;   //アトリビュートの変更が可能な場合はtrueを返す。
    }
};
