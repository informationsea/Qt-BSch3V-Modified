...
public:
    ...
    virtual bool readCe3(SReadCE3&,const SPoint* ,SXBSchDoc* ) {
        return false;
    }
    //    virtual bool readCe3(SReadCE3&,const SPoint* ){return false;}
    virtual bool writeCe3(SWriteCE3& ,const SPoint* ) {
        return false;
    }
    ...
    virtual void setY1(int y1) {
        m_p1.setY(y1);
    }
    virtual void setX2(int x2) {
        m_p2.setX(x2);
    }
    virtual void setY2(int y2) {
        m_p2.setY(y2);
    }
    virtual void setP1(const SPoint& p) {
        m_p1 = p;
    }
    virtual void setP2(const SPoint& p) {
        m_p2 = p;
    }
    //座標の取得
    int x1() {
...
public:
    ...
    virtual void rotate();
    //点を指定して選択重要度を返す
    virtual int qSelectWeight(const SPoint&) {
        return 0;
    }
    //選択フラグをセットする
    ...
    //範囲を指定して選択を行う
    virtual unsigned testSelection(const SRect& ) {
        return 0;
    }
    //範囲を選択してドラッグ選択を行う
    virtual unsigned testSelectionForDrag(const SRect& ) {
        return 0;
    }
    //描画が必要かどうかを返す
    virtual bool qRedraw(const SRect& ) {
        return 0;
    }
    //ピン数を返す
    ...
    //n番目のピンの位置を得る。
    virtual SPoint pinPosition(int) const {
        return SPoint(-1,-1);
    }
    virtual SRect area() {
        return SRect(-1,-1,0,0);
    }
    virtual SRect rotateRect() {
        return SRect(-1,-1,0,0);
    }
